...
Actually, @alg-curve doesn't need the family to be complete. Indeed, if $R_k=R_{(i,i)}=P_i$ is a leaf such that $\zeta_k=|P_i|$, the condition of line 12 of @alg-curve is always true, except for the last element of $P_i$ that is added to $S_t$. At that point, because the elements of $P_i$ have been exhausted, $P_i$ won't ever be visited again, so adding it to $\cK^-$ is irrelevant. Hence, if the family is not complete and $P_i$ is missing from the family, the algorithm behaves exactly as if it was present after all.
:::
